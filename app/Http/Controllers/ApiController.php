...
class ApiController extends Controller {
    ...
    public function atualizaOdds($idevent){
        ...
        $response = Http::get('https://api.betsapi.com/v1/bet365/start_sp?token=79193-eSO2iA6GxPNNey&FI='.$events->bet365_id.'');
    ...
    public function recuperaUpcomingEvents($idesporte, $pais){
        ...
        do{
            $response = Http::get('https://api.betsapi.com/v1/events/upcoming?token=79193-eSO2iA6GxPNNey&sport_id='.$idesporte.'&cc='.$pais.'&page='.$i.'');
                ...
                if(count($json->results) > 0){
                    ...
                    foreach($json->results as $dados_json){
                        ...
                        if(isset($dados_json->bet365_id)){
                            ...
                            if(isset($dados_json->bet365_id)){
                                $events->bet365_id = $dados_json->bet365_id;
                            }
    ...
    public function viewRecuperaLigasPorEsporte(){
        for($i = 1; $i < 100; $i++){
            $response = Http::get('https://api.betsapi.com/v1/league?token=79193-eSO2iA6GxPNNey&sport_id=1&page='.$i.'');
